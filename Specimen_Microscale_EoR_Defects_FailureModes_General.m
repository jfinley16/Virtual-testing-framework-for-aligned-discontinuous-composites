...
% Specimen_Microscale_EoR_Defects_FailureModes_General(repeat_counter,Composite_index,lf,lspecimen,n_rows0,n_columns0,pc,pInterface,pcFailed,pg,pgPartial,pgFailed,misalignment_switch,rng_switch,failure_modes_switch,parallelisation_flag)
...
function Specimen_Microscale_EoR_Defects_FailureModes_General(repeat_counter,Composite_index,lf,lspecimen,n_rows0,n_columns0,pc,pInterface,pcFailed,pg,pgPartial,pgFailed,misalignment_switch,rng_switch,failure_modes_switch,parallelisation_flag)
...
[dc,dg,ec,EcAvg,EcCoV,eg,EgAvg,EgCoV,G,GiicmAvg,gmAvg,lcWei,lgWei,mcWei,mgWei,Sc,Sg,SmAvg,SmCoV,Tu0,Vc,Vf]=load_inputs(Composite_type);
...
DTm=(Tc-Tg)*(t1-t2);
    ...
    if pcFailed>0 || pgFailed>0
            nc=round(nc*(1+pcFailed));
            ng=round(ng*(1+pgFailed));
            nf=round(nc+ng);
            n_rows=round(sqrt(nf*(n_rows0/n_columns0)));
            n_columns=round(sqrt(nf*(n_columns0/n_rows0)));
    else
        n_rows=n_rows0;
...
% set max strain as 2* max strain to failure of fibres
max_strain=max(2*max(ec),2*max(eg));
...
if parallelisation_flag==1 && floor(lspecimen/lf)>1
    parfor i=1:nRVE
        ...
        [EE{i},MSScurves{i},alphag(i),alphac(i),MminOverlap{i},MminOverlapType{i},crack_count{i},RVE_data{i},tm_temp(i)]=RVE_Microscale_EoR_SPupdates_Defects_FailureModesNewMethod(i*repeat_counter,deltaEpsilon,Vf,lf,pc,pInterface,pcFailed,pg,pgPartial,pgFailed,HalfLengthC,HalfLengthG,dg,EgAvg,EgCoV,Sg,dc,EcAvg,EcCoV,Sc,Vc,G,RVEg,first_fibre_overlap,n_rows,n_columns,HiPerDiF_fuzziness,Fuzzy_Factor,x_section_arrangement,max_strain,GiicmAvg,SmAvg,SmCoV,gmAvg,c_width,n_columns_c,mcWei,mgWei,lcWei,lgWei,Sc_frag_const,lf_SPSL,lf_hybrid,libGen,AA,AAE,AAde, AB,ABE,ABde, BB,BBE,BBde, deltaSigma, lLib,rng_switch,failure_modes_switch);
...
Specimen_dataset.Material_properties = Material_properties(dc,ec,EcAvg,EcCoV,lcWei,mcWei,Sc,dg,eg,EgAvg,EgCoV,lgWei,mgWei,Sg,G,GiicmAvg,gmAvg,SmAvg,SmCoV,Tu0,Vc,Vf);

save([Composite_type num2str(round(lf*1000)) '_' num2str(round(pc*100)) '_' num2str(round(pInterface*100)) '_' num2str(round(pcFailed*100)) '_' num2str(round(pg*100)) '_' num2str(round(pgPartial*100)) '_' num2str(round(pgFailed*100)) '_Specimen_' num2str(repeat_counter)],'Specimen_dataset','-v7.3')
